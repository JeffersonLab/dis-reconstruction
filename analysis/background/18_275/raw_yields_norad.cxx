...
const double s_cm = 4.*18.*275; //beam energy combination
...
int main(int argc, char **argv){
    ...
    for(int ihist=0;ihist<nEta;ihist++){
        ...
        h_se_5[ihist] = new TH1D(Form("h_se_5[%d]",ihist),"",100,-40,4);
        h_se_5[ihist]->SetLineColor(kGreen);h_se_5[ihist]->SetLineWidth(2);

        h_npi_5[ihist] = new TH1D(Form("h_npi_5[%d]",ihist),"",100,-40,4);
   ...
   for(int i=0;i<300;i++){
        t->Add(Form("/eic/data/baraks/pythiaeRHIC/outfiles/other_studies/18_275/fullQ2/ep_minbias_%d.root",i));
    }
    ...
    for(int iEvent=0;iEvent<nevents;iEvent++){
        ...
        if(part_se!=NULL && part_se->GetP()>0.1 ){ //Only consider electrons that have P>0.1 GeV/c
            ...
            h_se_5[bin_se]->Fill( empztot - (2.*18.) );       
        }
        ...
        for(int iPion=0;iPion<num_npi;iPion++){
            if( p_npi[iPion]>0.1 ){ //Only consider pions that have P>0.1 GeV/c
                ...
                h_npi_5[bin_npi[iPion]]->Fill( empztot - (2.*18.) );
            }
        }
    ...
    double mom_low[] = {0.916,0.916,1.022,1.800,7.550,50.038};
    TLine *linea[nEta];
    ...
    double mom_low_fine[] = {5.617,0.916,0.916,0.945,1.022,1.231};
    ...
    double mom_low_finer[] = {1.022,1.171,1.394};
    ...
    TH1 *hframe1 = gPad->DrawFrame(5E-5,1E-16,1.1,1E4);
    hframe1->SetTitle("18 GeV e^{-} on 275 GeV p");
    ...
    TF1 *f1a = new TF1("f1a",W_limit_y,5E-5,1,0);
    f1a->SetLineColor(kRed);f1a->SetLineWidth(3);
    f1a->Draw("Same");

    TF1 *f1b = new TF1("f1b",photo_limit_y,5E-5,1,0);
    f1b->SetLineColor(kGreen);f1b->SetLineWidth(3);
    f1b->Draw("same");

    TLatex *tex1a = new TLatex(2E-4,1E2,"W > 2 GeV Limit");
    tex1a->SetTextColor(kRed);tex1a->SetTextFont(42);
    tex1a->Draw();

    TLatex *tex1b = new TLatex(2E-2,1E-12,"Q^{2} > #frac{m_{e}^{2} y^{2}}{1-y} Limit");
    ...
    gPad->SetLogx();gPad->SetLogy();
    gPad->SetTickx();gPad->SetTicky();
    TH1 *hframe2 = gPad->DrawFrame(1E-16,1E-16,1.1,1E4);
    hframe2->SetTitle("18 GeV e^{-} on 275 GeV p");
    ...
    //Redraw with limit functions
    TF1 *f2a = new TF1("f2a",W_limit_x,1E-16,1,0);
    f2a->SetLineColor(kRed);f2a->SetLineWidth(3);
    f2a->Draw("Same");

    TF1 *f2b = new TF1("f1b",photo_limit_x,1E-16,1,0);
    f2b->SetLineColor(kGreen);f2b->SetLineWidth(3);
    f2b->Draw("same");

    TLatex *tex2a = new TLatex(5E-6,1E-6,"W > 2 GeV Limit");
    tex2a->SetTextColor(kRed);tex2a->SetTextFont(42);
    tex2a->Draw();

    TLatex *tex2b = new TLatex(1E-13,1E0,"Q^{2} < #frac{x^{2} s^{2}}{m_{e}^{2} + xs} (Q^{2} > #frac{m_{e}^{2} y^{2}}{1-y}) Limit");
    ...
    TLatex *tex3a = new TLatex(0.15,4E6,"18 GeV e^{-} on 275 GeV p");
    ...
    TLatex *tex3e = new TLatex(2,1.5E6,"P_{min.} Value");
    ...
    TLatex *tex5a = new TLatex(0.5,3E7,"18 GeV e^{-} on 275 GeV p");
    ...
    TLatex *tex6a = new TLatex(1,3E7,"18 GeV e^{-} on 275 GeV p");
    ...
    TLatex *tex7a = new TLatex(0.1,3E7,"18 GeV e^{-} on 275 GeV p");
    ...
    TLatex *tex8a = new TLatex(-2.8,3E7,"18 GeV e^{-} on 275 GeV p");
    ...
    TLatex *tex9a = new TLatex(-35,3E7,"18 GeV e^{-} on 275 GeV p");
    tex9a->SetTextColor(kBlack);tex9a->SetTextFont(42);

    TLatex *tex9b = new TLatex(-35,3E7,"Scattered Electron, p>0.1 GeV/c");
    tex9b->SetTextColor(kGreen);tex9b->SetTextFont(42);

    TLatex *tex9c = new TLatex(-35,9E6,"Negative Pions, p>0.1 GeV/c");
    tex9c->SetTextColor(kBlue);tex9c->SetTextFont(42);

    TLatex *tex9d = new TLatex(-35,2.5E6,"Sum particles w/|#eta|<3.5");
    ...
    for(int iCan=0;iCan<nEta;iCan++){
        ...
        hframe9[iCan] = gPad->DrawFrame(-40,0.1,4,2E8);
