...
void access_smeared_matrix(){
  ...
  //Load ROOT File
  TFile *f = new TFile("/eic/data/baraks/pythiaeRHIC/outfiles/yellow_report/5_41/ep_5_41_newtune_33_matrixsmear.root");
  ...
  for(Int_t i=0;i<10;i++){
	...
	printf("For Event %d, particle %d, is P smeared? %d \n",i,j,particle_s->IsPSmeared());
	printf("For Event %d, particle %d is detected with theta = %.3f degrees\n",i,j,theta[j]);
	printf("For Event %d, particle %d, is Theta smeared? %d \n",i,j,particle_s->IsThetaSmeared());
