...
void systematic_tables(){
    ...
    for(int i=0;i<15;i++){
        t->Add(Form("/eic/data/baraks/pythiaeRHIC/outfiles/yellow_report/18_110/ep_18_110_newtune_%d.root",i));
    }

    for(int i=0;i<30;i++){
        t->Add(Form("/eic/data/baraks/pythiaeRHIC/outfiles/yellow_report/additional/18_110/ep_18_110_newtune_%d.root",i));
    }
    ...
    double cross_tot = 1.33E9; //Total Cross Section in fb
    ...
    for(int i=0;i<nbins;i++){
        for(int j=0;j<nbins_Q2;j++){
            ...
            if(yield[i][j]>0) { 
                ...
                //Print to files
                if(yield[i][j]>1 && cut_lr[i][j] && cut_ul[i][j] && Q2_center[j]>Q2_min_good){
