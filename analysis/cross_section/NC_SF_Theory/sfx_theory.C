...
//Assumes file is in format grid number,Q2,x,y (plot x vs. x*y)
TGraph* Theory_Graph(const char* infile, int grid_set=0, double Q2_set=1, 
  ...
  while(in.good()){
    ...
    if( in.good() && grid==grid_set && fabs(Q2-Q2_set)<1E-2 ){
      ...
      if(abs)
        gr->SetPoint(counter,x,fabs(x*y));
      else
        gr->SetPoint(counter,x,x*y);
...
void sfx_theory(){
    ...
    for(int i=0;i<4;i++){
        ...
        gPad->SetTickx();
        gPad->SetTicky(); 
        hframe1[i] = gPad->DrawFrame(1E-4,0,1,0.1);
        hframe1[i]->GetXaxis()->SetTitle("x");hframe1[i]->GetXaxis()->CenterTitle();
        hframe1[i]->GetYaxis()->SetTitle("xF_{2,NC}");hframe1[i]->GetYaxis()->CenterTitle();
        ...
        tex1[i] = new TLatex(1E-3,0.08,Form("Q^{2} = %.0f GeV^{2}",Q2[i]));
        ...
        gPad->SetTickx();
        gPad->SetTicky(); 
        hframe2[i] = gPad->DrawFrame(1E-4,-0.01,1,0.01);
        hframe2[i]->GetXaxis()->SetTitle("x");hframe2[i]->GetXaxis()->CenterTitle();
        hframe2[i]->GetYaxis()->SetTitle("xF_{L,NC}");hframe2[i]->GetYaxis()->CenterTitle();
        ...
        gPad->SetTickx();
        gPad->SetTicky(); 
        hframe3[i] = gPad->DrawFrame(1E-4,0,1,0.035);
        hframe3[i]->GetXaxis()->SetTitle("x");hframe3[i]->GetXaxis()->CenterTitle();
        hframe3[i]->GetYaxis()->SetTitle("xF_{3,NC}");hframe3[i]->GetYaxis()->CenterTitle();
        ...
        if(i==0)c1[i]->Print("sfx_theory.pdf[");
        c1[i]->Print("sfx_theory.pdf");
        if(i==3)c1[i]->Print("sfx_theory.pdf]");
    }
}