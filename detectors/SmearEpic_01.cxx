...
// B.S. 10/14/2022
// For EPIC studies, I did the following:
// 1) Extended the acceptance to -4 < eta < 4 for all detector categories.
// 2) Added constant terms for the EmCals
// 3) Added non-zero theta and phi resolutions.
// 4) Edited tracking separation regions a bit to be consistent with Ecal
...
Smear::Detector BuildEpic_01() {
  ...
  // --------------
  // hadrons
  Smear::Acceptance::Zone AngleZoneHadronic(ThetaFromEta ( 4.0 ),ThetaFromEta ( -4.0 ));
  Smear::Device SmearThetaHadronic(Smear::kTheta, "0.01 / ( P * TMath::Sqrt(TMath::Sin(theta)) )");
  ...
  Smear::Device SmearPhiHadronic(Smear::kPhi, "0.01");
  ...
  Smear::Acceptance::Zone AngleZoneMuon(ThetaFromEta ( 4.0 ),ThetaFromEta ( -4.0 ));
  Smear::Device SmearThetaMuon(Smear::kTheta, "0.01 / ( P * TMath::Sqrt(TMath::Sin(theta)) )");
  SmearThetaMuon.Accept.AddZone(AngleZoneMuon);
  SmearThetaMuon.Accept.AddParticle(13);
  SmearThetaMuon.Accept.AddParticle(-13);
  det.AddDevice(SmearThetaMuon);

  Smear::Device SmearPhiMuon(Smear::kPhi, "0.01");
  ...
  // electromagnetic
  Smear::Acceptance::Zone AngleZoneEmcal(ThetaFromEta ( 4.0 ),ThetaFromEta ( -4.0 ));
  Smear::Device SmearThetaEmcal(Smear::kTheta, "0.01 / ( P * TMath::Sqrt(TMath::Sin(theta)) )");
  ...
  Smear::Device SmearPhiEmcal(Smear::kPhi, "0.01");
  ...
  // eta = -4.0 --  -2.0
  // sigma_p/p ~ 0.1% p+0.5%
  Smear::Acceptance::Zone TrackBack1Zone(ThetaFromEta ( -2.0 ),ThetaFromEta ( -4.0 ));
  ...
  Smear::Acceptance::Zone TrackBack2Zone(ThetaFromEta ( -1 ),ThetaFromEta ( -2.0 ));
  ...
  // eta = 2.5 -- 4.0
  // sigma_p/p ~ 0.1% p+2.0%
  Smear::Acceptance::Zone TrackFwd1Zone(ThetaFromEta ( 4.0 ),ThetaFromEta ( 2.5 ));
  ...
  // eta = -4.0 -- -2
  // stoch. = 2%, const. = 1%
  Smear::Acceptance::Zone EmcalBackZone(ThetaFromEta ( -2 ),ThetaFromEta ( -4.0 ));
  ...
  // Forward
  // eta = -1 -- 4.0
  // stoch. = 10-12%, use 12%, const. = 1%
  Smear::Acceptance::Zone EmcalFwdZone(ThetaFromEta ( 4.0 ),ThetaFromEta ( -1 ));
  Smear::Device EmcalFwd(Smear::kE, "sqrt( pow ( 0.01*E,2 ) + pow( 0.12,2)*E)");
  ...
  // Smear::Acceptance::Zone acceptpid(ThetaFromEta(4.0),ThetaFromEta(-4.0));
  // Smear::PerfectID pid;
  ...
  // eta = -4.0 -- -1
  // stoch. = 50%
  Smear::Acceptance::Zone HcalBackZone(ThetaFromEta ( -1 ),ThetaFromEta ( -4.0 ));
  ...
  // eta = 1 -- 4.0
  // stoch. = 50%
  Smear::Acceptance::Zone HcalFwdZone(ThetaFromEta ( 4.0 ),ThetaFromEta ( 1 ));
