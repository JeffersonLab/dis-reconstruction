...
echo "-----------------------------------"
echo "Running DJANGOH Simulation for eAu Collider!!!"
echo "..."
echo ""


OUTFILE1=outfiles/djangoh.NC.Au.18x110_evt.dat
if test -f "$OUTFILE1"; then
	rm -f "$OUTFILE1"
fi
OUTFILE2=outfiles/djangoh.NC.Au.18x110_out.dat
if test -f "$OUTFILE2"; then
	rm -f "$OUTFILE2"
fi
OUTFILE3=outfiles/djangoh.NC.Au.18x110_smp.dat
if test -f "$OUTFILE3"; then
        rm -f "$OUTFILE3"
fi

djangoh < ep.Rad=0.NC.Au.in > logfiles/ep.Rad=0.NC.Au.log

echo "Completed Simulation!!!"
echo ""

echo "Making Output ROOT File..."
root -l -b -q 'make_tree.C("djangoh.NC.Au.18x110_evt.dat")'
echo "Done!!!"
